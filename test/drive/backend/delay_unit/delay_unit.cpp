...
void gen_delay_unit_context(DelayUnitContext *context) {
  ...
  for (int i = 0; i < 3; i++) {    
    context->input_chip_read_req_with_times[i].raw = RandomGen::rand_chip_read_req();
    context->input_chip_read_resp_with_times[i].raw = RandomGen::rand_chip_read_resp();
    context->input_chip_write_req_with_times[i].raw = RandomGen::rand_chip_write_req();
...
void test_delay_unit_impl(bool use_user_defined_delay_cycles,
  ...
  std::vector<Chip_Read_Req> result_chip_read_reqs;
   ...
   drain_queue(context.delayed_chip_read_resp_queue.get(), &result_chip_read_resps);
   ASSERT_EQ(result_chip_read_resps.size(), 0);
   drain_queue(context.delayed_chip_read_req_queue.get(), &result_chip_read_reqs);
   ASSERT_EQ(result_chip_read_reqs.size(), 0);
   drain_queue(context.delayed_chip_write_req_queue.get(), &result_chip_write_reqs);
