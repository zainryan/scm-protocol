...
inline void cmd_dispatcher_impl(
  ...
  if (command_queue->read_nb(command)) {
    ...
    if (bank_id == 0) {
      if (command.is_read) {
        read_req_handler(command, chip_read_req_queue_0,
      ...
      if (command.is_read) {
        read_req_handler(command, chip_read_req_queue_1,
      ...
      if (command.is_read) {
        read_req_handler(command, chip_read_req_queue_2,
      ...
      if (command.is_read) {
        read_req_handler(command, chip_read_req_queue_3,
